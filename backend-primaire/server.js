...
const allowedOrigins = [
    'http://localhost:3000',
    'http://127.0.0.1:3000',
    'https://trayebernard-primaire.com',
    'http://localhost:3001',
    'http://127.0.0.1:3001'
];
...
const corsOptions = {
    origin: function(origin, callback) {
        ...
        // En production, retourner une erreur pour les origines non autorisées
...
app.use((req, res, next) => {
    ...
    if (origin && origin.includes('localhost')) {
        console.log(`[CORS] Requête ${req.method} depuis ${origin} vers ${req.path}`);
    }
    next();
});
...
// Middleware pour gérer les requêtes OPTIONS (preflight) de manière explicite
app.use((req, res, next) => {
    ...
    if (req.method === 'OPTIONS') {
        ...
        if (isAllowed) {
            ...
            console.log(`[CORS] Preflight autorisé pour ${origin}`);
            return res.sendStatus(200);
        } else {
            console.warn(`[CORS] Preflight refusé pour ${origin}`);
...
// Route de debug pour un étudiant spécifique (bulletin)
app.get('/api/debug/student-bulletin/:studentId', async(req, res) => {
    try {
        ...
        console.log('[DEBUG BULLETIN] Étudiant:', studentId, 'Trimestre:', trimester, 'Année:', school_year, 'Composition:', composition_id);

        // 1. Info étudiant et classe
        ...
        if (studentInfo.length === 0) {
            return res.json({ error: 'Étudiant non trouvé ou non inscrit' });
        }

        const student = studentInfo[0];
        console.log('[DEBUG BULLETIN] Info étudiant:', student);
        ...
        // 2. Toutes les notes de l'étudiant (publiées et non publiées)
        const [allGrades] = await pool.query(`
            SELECT 
...
app.get('/api/test/bulletin/:studentId', async(req, res) => {
    try {
        ...
        // 1. Vérifier l'inscription de l'étudiant
        const [studentInfo] = await pool.query(`
            SELECT 
                s.first_name, s.last_name, s.registration_number,
                c.id as class_id, c.name as class_name
        ...
        const student = studentInfo[0];
        console.log('[TEST BULLETIN] Étudiant trouvé:', student);
...
app.get('/api/test/bulletin-frontend/:studentId', async(req, res) => {
    try {
        ...
        console.log(`[FRONTEND TEST] === Début test bulletin étudiant ${studentId} ===`);
...
app.get('/api/test/bulletin-simple/:studentId', async(req, res) => {
    try {
        ...
        console.log(`[SIMPLE TEST] === Test bulletin étudiant ${studentId} ===`);
        ...
        if (studentInfo.length === 0) {
            console.log(`[SIMPLE TEST] Aucun étudiant trouvé pour ID ${studentId}`);
            return res.json({ error: 'Étudiant non trouvé', studentId, school_year });
        }

        const student = studentInfo[0];
        console.log(`[SIMPLE TEST] Étudiant trouvé:`, student);
...
app.get('/api/test/bulletin-noauth/:studentId', async(req, res) => {
    try {
        ...
        console.log(`[NO AUTH TEST] Test bulletin pour étudiant ${studentId}`);
...
app.get('/api/test/student-grades/:studentId', async(req, res) => {
    try {
        ...
        console.log(`[TEST GRADES] Test pour étudiant ${studentId}, année ${school_year}`);
...
app.get('/api/test/student-bulletin-grades/:studentId', async(req, res) => {
    try {
        ...
        console.log(`[BULLETIN GRADES] Test pour étudiant ${studentId}, année ${school_year}`);
