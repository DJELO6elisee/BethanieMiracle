...
const classController = {
    ...
    // Retourne TOUS les étudiants inscrits (peu importe le statut)
    // Si un élève est dans enrollments pour cette classe et année, il est considéré comme inscrit
    getClassStudents: async(req, res) => {
        ...
        try {
            ...
            console.log(`[CLASS CONTROLLER] Nombre d'étudiants récupérés: ${students.length} pour la classe ${classId}, année ${school_year || 'toutes'}`);
            ...
            console.log(`[CLASS CONTROLLER] Total dans enrollments: ${totalEnrolled}, étudiants retournés: ${students.length}`);

            if (totalEnrolled !== students.length) {
                console.warn(`[CLASS CONTROLLER] ⚠️ ATTENTION: ${totalEnrolled} étudiants dans enrollments mais seulement ${students.length} retournés. Vérifiez les jointures.`);
            }

            res.json(students);
        } catch (error) {
            console.error('[CLASS CONTROLLER] Erreur lors de la récupération des étudiants:', error);
            res.status(500).json({ message: error.message });
        }
    },
    ...
    // Notifier les étudiants d'une classe (notification interne, pas d'email)
    notifyStudents: async(req, res) => {
        ...
        try {
            ...
            res.json({ message: 'Notification interne : les étudiants verront leurs notes sur leur dashboard.' });
        } catch (error) {
            console.error(error);
