...
const authController = {
    ...
    registerAdmin: async(req, res) => {
        try {
            ...
            // Validation du rôle
            const allowedRoles = ['admin', 'secretary', 'éducateur', 'comptable', 'comunicateur', 'informaticien'];
            if (!role || !allowedRoles.includes(role)) {
                return res.status(400).json({
                    status: 'error',
                    message: 'Rôle invalide. Les rôles autorisés sont: admin, secretary, éducateur, comptable, comunicateur, informaticien'
                });
            }
    ...
    getProfile: async(req, res) => {
        try {
            ...
            if (role === 'student') {
                ...
                console.log('[getProfile] Étudiant trouvé pour user_id', userId, ':', student);

                if (student.length > 0) {
                    profileData = { student: student[0] };
                    console.log('[getProfile] Données étudiant ajoutées au profil:', student[0]);
                } else {
                    console.log('[getProfile] AUCUN ÉTUDIANT TROUVÉ pour user_id', userId);
                }
    ...
    getCurrentPassword: async(req, res) => {
        try {
            ...
            const [user] = await pool.query(
                'SELECT password FROM users WHERE id = ? AND role IN (?, ?, ?)', [userId, 'admin', 'secretary', 'comunicateur']
            );
    ...
    updateOwnProfile: async(req, res) => {
        ...
        try {
            // Vérifier que l'utilisateur existe et est un admin/secretary/comunicateur
            const [existingUser] = await pool.query(
                'SELECT * FROM users WHERE id = ? AND role IN (?, ?, ?)', [userId, 'admin', 'secretary', 'comunicateur']
            );
    ...
    // Connexion avec code et nom (étudiant ou professeur)
    loginWithCode: async(req, res) => {
        ...
        try {
            ...
            console.log('Résultat recherche étudiant:', students);
            if (students.length > 0) {
                ...
                console.log('Connexion étudiant, token:', token);
                return res.json({
                    status: 'success',
                    message: 'Connexion réussie',
                    data: {
            ...
            console.log('Aucun étudiant ou professeur trouvé pour ce code/nom');
    ...
    getAdmins: async(req, res) => {
        try {
            const [admins] = await pool.query(`
                ...
                FROM users
                WHERE role IN ('admin', 'secretary', 'éducateur', 'comptable', 'comunicateur', 'informaticien')
    ...
    deleteAdmin: async(req, res) => {
        ...
        try {
            ...
            const [existingUser] = await pool.query(
                'SELECT * FROM users WHERE id = ? AND role IN (?, ?, ?, ?, ?)', [id, 'admin', 'secretary', 'éducateur', 'comptable', 'comunicateur']
            );
    ...
    debugParentChildren: async(req, res) => {
        ...
        try {
            ...
            // 1. Rechercher tous les étudiants avec ce parent_code
            ...
            console.log('Étudiants trouvés par parent_code:', studentsByCode);

            // 2. Rechercher tous les étudiants avec ce parent_email
            ...
            console.log('Étudiants trouvés par parent_email:', studentsByEmail);
            ...
            if (req.query.parent_name) {
                ...
                console.log('Étudiants trouvés par nom de parent:', studentsByName);
            }
    ...
    createParentAccount: async(req, res) => {
        ...
        try {
            ...
            // Vérifier qu'il y a bien un étudiant avec ce parent_code
            const [students] = await pool.query(
            ...
            if (students.length === 0) {
                return res.status(404).json({
                    status: 'error',
                    message: 'Aucun étudiant trouvé avec ce code parent et cet email'
                });
            }
