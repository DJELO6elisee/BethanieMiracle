...
const deleteEducationLevel = async(req, res) => {
        try {
            ...
            // Vérifier s'il y a des étudiants associés via les classes
        ...
        `, [id]);
            ...
            `, [id]);
                ...
                if (students[0].count > 0) {
                    errorMessage += `- ${students[0].count} étudiant(s) (exemples: ${studentDetails.map(s => `${s.first_name} ${s.last_name}`).join(', ')})`;
...
// Récupérer les versements détaillés d'un niveau avec statistiques des étudiants
const getLevelInstallmentsDetails = async(req, res) => {
    try {
        ...
        // Récupérer les statistiques globales des étudiants de ce niveau
        ...
        // Récupérer les détails des versements des étudiants
...
const getEducationLevelUsage = async(req, res) => {
    try {
        ...
        // Statistiques des versements des étudiants
