...
const adminStorage = multer.diskStorage({
    destination: (req, file, cb) => {
        // Déterminer le dossier de destination basé sur l'étudiant ou la classe
        let uploadPath = 'protected_uploads';

        if (req.body.student_id) {
            // Si c'est pour un étudiant spécifique, créer un dossier avec son nom
...
// Configuration du stockage pour les médias étudiants
...
// Configuration de multer pour les médias étudiants
...
// Fonction pour créer la structure de dossiers pour un étudiant
...
const mediaController = {
    ...
    getAllMedia: async(req, res) => {
        try {
            ...
            // Ajouter les noms complets des étudiants et les chemins de fichiers
    ...
    getProtectedMedia: async(req, res) => {
        try {
            ...
            if (media.length > 0) {
                ...
                if (media.length > 0) {
                    // Les médias étudiants sont stockés dans protected_uploads comme les médias admin
    ...
    // Upload de média étudiant (existant)
    uploadStudentMedia: async(req, res) => {
        try {
            ...
            console.error('Erreur upload média étudiant:', error);
            res.status(500).json({ message: 'Erreur lors de l\'upload du média' });
        }
    },

    // Récupérer les médias d'un étudiant (existant)
    ...
    // Upload de média en masse pour les étudiants
    uploadBulkStudentMedia: async(req, res) => {
        try {
            ...
            console.error('Erreur upload média en masse étudiant:', error);
