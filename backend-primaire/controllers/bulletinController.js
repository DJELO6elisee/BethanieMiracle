 ...
 * - getClassStudentsWithBulletins: Récupère les étudiants avec leurs bulletins
 * 
...
const bulletinController = {
    ...
    publishBulletin: async(req, res) => {
      ...
      `;
            ...
            // Récupérer tous les parents des étudiants de cette classe (tous les étudiants inscrits, peu importe le statut)
    ...
    // Récupérer les étudiants d'une classe avec leurs bulletins
    getClassStudentsWithBulletins: async(req, res) => {
        ...
        try {
            ...
            try {
                // Étape 1a: Mettre à jour les étudiants avec NULL school_year vers l'année courante
                ...
                // Continuer quand même pour récupérer les étudiants
            } finally {
                if (connection) {
                    connection.release();
                }
            ...
            // Étape 2: Récupérer TOUS les étudiants avec LEFT JOIN pour être sûr de ne perdre aucun élève
            ...
            // On inclut aussi les étudiants avec NULL school_year au cas où un script les aurait ajoutés
            ...
            console.log(`[BULLETIN CONTROLLER] 📊 Total étudiants TOUTES années confondues: ${studentsAllYears.length}`);
            ...
            // Si on a trouvé des étudiants avec une autre année scolaire, les mettre à jour automatiquement
            ...
            if (studentsWrongYear.length > 0) {
                console.log(`[BULLETIN CONTROLLER] ⚠️ ${studentsWrongYear.length} étudiant(s) trouvé(s) avec une autre année scolaire. Mise à jour automatique...`);
                ...
                try {
                    ...
                    // Recharger les étudiants après la mise à jour
            ...
            console.log(`[BULLETIN CONTROLLER] Nombre d'étudiants récupérés: ${students.length} pour la classe ${classId}, année ${school_year}`);

            // Vérification: Compter tous les étudiants dans enrollments pour cette classe
            ...
            // Compter aussi TOUS les étudiants de la classe (toutes années confondues) pour diagnostic
            ...
            console.log(`[BULLETIN CONTROLLER] Total dans enrollments: ${totalEnrolled}, étudiants retournés: ${students.length}`);
            ...
            if (totalEnrolled !== students.length) {
                console.warn(`[BULLETIN CONTROLLER] ⚠️ ATTENTION: ${totalEnrolled} étudiants dans enrollments mais seulement ${students.length} retournés.`);
                ...
                if (missingStudents.length > 0) {
                    console.warn(`[BULLETIN CONTROLLER] ⚠️ ${missingStudents.length} étudiant(s) manquant(s):`, missingStudents);
                }
            }
