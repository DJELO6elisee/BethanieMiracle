...
const processIntelligentPayment = async(connection, studentId, amount, paymentDate, paymentMethod, schoolYear, description = null) => {
    try {
        console.log(`[INTELLIGENT PAYMENT] Traitement du paiement de ${amount} FCFA pour l'étudiant ${studentId}`);

        // 1. Récupérer les informations du niveau d'études de l'étudiant
        ...
        if (studentInfo.length === 0) {
            throw new Error('Étudiant non trouvé ou non inscrit pour cette année');
        }
        ...
        // 3. Vérifier s'il y a déjà des versements créés pour cet étudiant
...
// Créer les versements pour un étudiant
const createStudentInstallments = async(connection, studentId, educationLevelId, enrollmentDate, schoolYear) => {
    try {
        ...
        // Créer les versements pour l'étudiant
        for (const levelInstallment of levelInstallments) {
            // Utiliser le montant fixe de la configuration si disponible, sinon calculer par pourcentage
...
// Récupérer le résumé financier d'un étudiant
    ...
    try {
        ...
        if (!student_id || !amount || Number(amount) <= 0) {
            return res.status(400).json({
                success: false,
                message: 'ID étudiant et montant valide requis'
            });
        }
