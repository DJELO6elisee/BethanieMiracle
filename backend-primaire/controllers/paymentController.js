...
const paymentController = {
        ...
        createPayment: async(req, res) => {
            ...
            if (!student_id || !amount || Number(amount) <= 0) {
                return res.status(400).json({ message: 'Un ID étudiant et un montant valide sont requis.' });
            }
        ...
        generateReceipt: async(req, res) => {
                try {
                    ...
                    // Récupérer les informations du paiement avec les détails de l'étudiant
            ...
            `, [payment.student_id, payment.school_year]);
                    ...
                    if (studentInfo.length > 0) {
                        ...
                        console.log('[generateReceipt] Aucune information trouvée pour l\'étudiant, tentative sans filtre d\'année...');
            ...
            `, [payment.student_id, payment.school_year]);
                ...
                <div style="text-align:center; margin-bottom: 15px;">
                    ${payment.child_photo ? `
                    <img src="https://trayebernard-primaire.com/api/students/photo/${encodeURIComponent(payment.child_photo)}" alt="Photo de ${payment.first_name} ${payment.last_name}" style="width:140px;height:140px;border-radius:50%;object-fit:cover;border:3px solid #1976d2;">
                    ` : `
        ...
        try {
            ...
            if (formattedDate) {
                ...
                if (inscriptionData.length > 0) {
                    ...
                    // Fallback : récupérer la première inscription de l'étudiant
                    [inscriptionData] = await pool.query(`
                        SELECT 
                            s.*,
                            c.name as classe,
                ...
                // Récupérer la première inscription de l'étudiant
                [inscriptionData] = await pool.query(`
                    SELECT 
                        s.*,
                        c.name as classe,
            ...
            if (tuitionAmount === 0) {
                ...
                if (levelInfo.length > 0) {
                    ...
                    console.log('[getInscriptionReceipt] Aucune information trouvée pour l\'étudiant');
                }
            } else {
    ...
    getPaymentReceipt: async(req, res) => {
        try {
            ...
            if (!id || !paymentId) {
                ...
                return res.status(400).json({ message: 'ID étudiant et ID paiement requis' });
            }
            ...
            if (studentInfo.length > 0) {
                ...
                console.log('[getPaymentReceipt] Aucune information trouvée pour l\'étudiant, tentative sans filtre d\'année...');
