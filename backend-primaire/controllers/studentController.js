...
// Fonction utilitaire pour vérifier si un étudiant appartient à un parent
...
const studentController = {
        ...
        getAllStudents: async(req, res) => {
                ...
                if (req.user.role === 'parent') {
                    ...
                    for (const student of parentStudents) {
                        try {
                            ...
                            console.error('[DEBUG] Erreur calcul financier pour étudiant', student.id, ':', finError);
            ...
            `;
                ...
                // Afficher uniquement les étudiants inscrits en ligne OU ayant une inscription active
                ...
                console.log('[DEBUG] Étudiants trouvés (avec doublons):', studentsWithDuplicates.length);
                if (req.user.role === 'parent') {
                    console.log('[DEBUG] Premiers étudiants trouvés:', studentsWithDuplicates.slice(0, 3));
                }
                ...
                if (req.user.role === 'parent') {
                    console.log('[DEBUG] Étudiants après suppression des doublons:', students.length);
                    console.log('[DEBUG] IDs des étudiants trouvés:', students.map(s => s.id));
                }

                // Ajout du champ reste_a_payer pour chaque étudiant
                ...
                // Debug: afficher les calculs pour le premier étudiant
                if (students.length > 0) {
                    const firstStudent = students[0];
                    console.log('[DEBUG DASHBOARD] Étudiant:', firstStudent.first_name, firstStudent.last_name);
        ...
        getStudentById: async(req, res) => {
            ...
            try {
                ...
                // Vérification de sécurité pour les parents : s'assurer que l'étudiant appartient au parent connecté
                if (req.user.role === 'parent') {
                    ...
                    if (studentCheck.length === 0) {
                        return res.status(403).json({ message: 'Vous n\'avez pas l\'autorisation de voir les informations de cet étudiant.' });
                    }
                }
        ...
        createStudent: async(req, res) => {
            ...
            try {
                ...
                // Hashage du mot de passe pour l'étudiant
                ...
                console.log('=== [DEBUG] Création de l\'utilisateur étudiant...');
                const [userResult] = await connection.query(
                    'INSERT INTO users (email, password, role) VALUES (?, ?, ?)', [studentEmail, hashedStudentPassword, 'student']
                );
                console.log('=== [DEBUG] Utilisateur étudiant créé, ID:', userResult.insertId);
                ...
                // Créer l'étudiant avec les infos parent
            ...
            `;
                ...
                console.log('=== [DEBUG] Étudiant créé, ID:', studentResult.insertId);

                const enrollmentYear = getCurrentSchoolYear();

                // Inscrire l'étudiant dans la classe souhaitée
                if (desired_class) {
        ...
        updateStudent: async(req, res) => {
            ...
            try {
                ...
                if (class_id) {
                    ...
                    // Vérifier si une inscription pour cette classe existe déjà pour cet étudiant
        ...
        deleteStudent: async(req, res) => {
            ...
            try {
                ...
                // Supprimer les dépendances avant l'étudiant
                ...
                // Supprimer les autres tables liées à l'étudiant
                try {
        ...
        getStudentGrades: async(req, res) => {
            const studentId = req.params.id;
            console.log(`[getStudentGrades V4] Début - Récupération des notes pour l'étudiant ID: ${studentId}`);
            console.log(`[getStudentGrades V4] Utilisateur connecté - ID: ${req.user.id}, Role: ${req.user.role}`);

            // Vérification de sécurité pour s'assurer qu'un étudiant ne peut voir que ses propres notes
            if (req.user.role === 'student') {
                try {
                    const [studentCheck] = await pool.query('SELECT id, first_name, last_name, user_id FROM students WHERE user_id = ?', [req.user.id]);
                    console.log(`[getStudentGrades V4] Étudiant trouvé pour user_id ${req.user.id}:`, studentCheck);
                    ...
                    if (!studentCheck.length || studentCheck[0].id !== Number(studentId)) {
                        ...
                        console.warn(`[getStudentGrades V4] Étudiant autorisé: ${studentCheck[0]?.id}, Étudiant demandé: ${studentId}`);
            ...
            if (req.user.role === 'parent') {
                try {
                    ...
                    console.log(`[getStudentGrades V4] Vérification parent pour étudiant ${studentId}:`, studentCheck);

                    if (studentCheck.length === 0) {
                        console.warn(`[getStudentGrades V4] Étudiant ${studentId} non trouvé`);
                        return res.status(404).json({ message: 'Étudiant non trouvé.' });
                    }
                } catch (secError) {
            ...
            `;
                ...
                if (grades.length === 0) {
                    console.log(`[getStudentGrades V4] Aucune note publiée trouvée pour l'étudiant ${studentId}`);
                ...
                `, [studentId]);
                    console.log(`[getStudentGrades V4] Notes non publiées pour l'étudiant ${studentId}:`, unpublishedGrades[0].count);
        ...
        finalizeRegistration: async(req, res) => {
            ...
            try {
                ...
                // 1. Récupérer les informations de l'étudiant avant mise à jour
                ...
                console.log('=== [DEBUG] Étudiant récupéré de la DB:', student);
                ...
                // 2. Mettre à jour l'étudiant avec les codes générés et les champs cantine
                ...
                // 3. Inscrire l'étudiant dans la classe pour l'année courante
                await connection.query(
        ...
        getStudentDetails: async(req, res) => {
            ...
            try {
                ...
                if (studentInfo.length > 0) {
                    console.log(`[getStudentDetails] Étudiant ${id}:`);
                ...
                console.log(`[getStudentDetails] Parent code pour étudiant ${id}:`, student.parent_code);
                ...
                if (!('parent_code' in student)) {
                    console.warn(`[getStudentDetails] ATTENTION: parent_code manquant dans la réponse pour l'étudiant ${id}`);
        ...
        // Obtenir l'emploi du temps de l'étudiant
        getStudentSchedule: async(req, res) => {
            const studentId = req.params.id;
            console.log(`[getStudentSchedule] Récupération de l'emploi du temps pour l'étudiant ID: ${studentId}`);
            ...
            if (req.user.role === 'parent') {
                try {
                    ...
                    if (studentCheck.length === 0) {
                        return res.status(403).json({ message: 'Vous n\'avez pas l\'autorisation de voir l\'emploi du temps de cet étudiant.' });
                    }
                } catch (secError) {
            ...
            // Vérification de sécurité pour s'assurer qu'un étudiant ne peut voir que son propre emploi du temps
            if (req.user.role === 'student') {
                try {
                    const [studentCheck] = await pool.query('SELECT id, user_id FROM students WHERE user_id = ?', [req.user.id]);
                    console.log(`[getStudentSchedule] Étudiant trouvé pour user_id ${req.user.id}:`, studentCheck);
            ...
            try {
                ...
                if (enrollment.length === 0) {
                    console.log(`[getStudentSchedule] Aucune classe active trouvée pour l'étudiant ${studentId}`);
                    return res.json({
        ...
        // Récupérer toutes les infos de l'étudiant connecté
        getMyDetails: async(req, res) => {
            ...
            try {
                ...
                if (students.length === 0) {
                    console.log('[DEBUG] RETURN (étudiant non trouvé)', students);
                    return res.status(404).json({ message: "Étudiant non trouvé" });
                }
        ...
        // Obtenir les absences d'un étudiant spécifique
        ...
        getCantineStudents: async(req, res) => {
            ...
            try {
                ...
                // Récupérer les étudiants de cantine
        ...
        // Récupérer les médias d'un étudiant (médias étudiants + médias admin)
        getStudentMedia: async(req, res) => {
            try {
                ...
                // Vérifier que l'utilisateur a le droit de voir les médias de cet étudiant
                if (req.user.role === 'parent') {
                    console.log('[DEBUG] Vérification des permissions pour parent');
                    // Pour les parents, vérifier que l'étudiant leur appartient (uniquement par parent_code)
                    const tokenStudentId = req.user.student_id;
                    if (!tokenStudentId || Number(tokenStudentId) !== Number(id)) {
                        return res.status(403).json({ message: 'Accès interdit.' });
                    }
                    ...
                    if (students.length === 0) {
                        ...
                        return res.status(403).json({
                            message: 'Vous n\'avez pas l\'autorisation de voir les médias de cet étudiant'
                        });
                    }
                }

                // Récupérer les médias étudiants (student_media)
                console.log('[DEBUG] Récupération des médias étudiants');
            ...
            `, [id]);
                console.log('[DEBUG] Médias étudiants trouvés:', studentMedia.length);

                // Récupérer les médias admin envoyés à cet étudiant (admin_media)
            ...
            `, [id]);
                ...
                // Récupérer les médias admin envoyés à la classe de l'étudiant
            ...
            `, [id]);
                ...
                const mediaWithUrls = allMedia.map(item => {
                    return {
                        ...item,
                        media_url: `https://trayebernard-primaire.com/api/media/${item.id}`,
                ...
                console.error('Erreur récupération médias étudiant:', error);
        ...
        // Rechercher un étudiant par matricule pour réinscription (sans filtre d'inscription active)
        searchStudentByRegistration: async(req, res) => {
            try {
                const { registration_number } = req.params;
                console.log('[DEBUG] Recherche étudiant par matricule:', registration_number);

                // Requête simple pour trouver l'étudiant par matricule, sans filtre d'inscription
                const query = `
                SELECT 
                    s.id,
                    s.first_name,
            ...
            `;
                ...
                if (students.length === 0) {
                    console.log('[DEBUG] Aucun étudiant trouvé avec le matricule:', registration_number);
                ...
                const student = students[0];
                console.log('[DEBUG] Étudiant trouvé:', student.first_name, student.last_name);
        ...
        // Récupérer l'emploi du temps hebdomadaire publié pour un étudiant
        getStudentWeeklySchedule: async(req, res) => {
            try {
                ...
                // Vérifier que l'étudiant appartient au parent connecté (si c'est un parent)
                if (req.user.role === 'parent') {
                    const tokenStudentId = req.user.student_id;
                    ...
                    if (studentCheck.length === 0) {
                        console.log(`[getStudentWeeklySchedule] Étudiant ${studentId} non trouvé`);
                        return res.status(404).json({ message: 'Étudiant non trouvé' });
                    }

                    console.log(`[getStudentWeeklySchedule] Accès autorisé pour l'étudiant ${studentId}`);
                }
                ...
                if (enrollmentResult.length === 0) {
                    console.log(`[getStudentWeeklySchedule] Aucune inscription trouvée pour l'étudiant ${studentId}`);
        ...
        // Obtenir le résumé de paiement d'un étudiant
        getStudentPaymentSummary: async(req, res) => {
            try {
                ...
                // Vérifier que l'utilisateur a le droit de voir les paiements de cet étudiant
                if (req.user.role === 'parent') {
                    ...
                    if (students.length === 0) {
                        ...
                        return res.status(403).json({
                            message: 'Vous n\'avez pas l\'autorisation de voir les paiements de cet étudiant'
                        });
                    }
                }
                ...
                if (enrollmentResult.length === 0) {
                    return res.status(404).json({ message: 'Aucune inscription active trouvée pour cet étudiant' });
                }
        ...
        // Récupérer l'historique des reçus d'un étudiant
        getReceiptHistory: async(req, res) => {
            try {
                ...
                // Vérifier que l'utilisateur a le droit de voir les reçus de cet étudiant
                if (req.user.role === 'parent') {
                    ...
                    if (students.length === 0) {
                        ...
                        return res.status(403).json({
                            message: 'Vous n\'avez pas l\'autorisation de voir les reçus de cet étudiant'
                        });
                    }
                }
        ...
        getInscriptionReceipt: async(req, res) => {
            ...
            `, [id, formattedDate, localDate]);
                    ...
                    // Si aucune inscription trouvée avec la date exacte, essayer de trouver la première inscription de l'étudiant
                    if (inscriptionData.length === 0) {
                ...
                `, [id]);
                        ...
                        if (fallbackData.length === 0) {
                            return res.status(404).json({ message: 'Aucune inscription trouvée pour cet étudiant' });
                        }
                ...
                <body>
                    <div class="header">
                        <div class="logo-container">
                            <img src="https://trayebernard-primaire.com/img/pages/logologo.webp" alt="Logo École" class="logo">
                        </div>
        ...
        // Récupérer le bulletin d'un étudiant pour un trimestre donné
        getStudentBulletin: async(req, res) => {
            try {
                ...
                // Vérifier que l'utilisateur a le droit de voir le bulletin de cet étudiant
                if (req.user.role === 'parent') {
                    ...
                    if (!tokenStudentId || Number(tokenStudentId) !== Number(id)) {
                        ...
                        return res.status(403).json({
                            message: 'Vous n\'avez pas l\'autorisation de voir le bulletin de cet étudiant'
                        });
                    }
                    ...
                    if (students.length === 0) {
                        console.log('[getStudentBulletin] Étudiant non trouvé');
                        return res.status(404).json({
                ...
                // 1. Récupérer les informations de l'étudiant et de sa classe
                ...
                if (studentInfo.length === 0) {
                    return res.status(404).json({ message: 'Étudiant non trouvé ou non inscrit pour cette année scolaire' });
                }
                ...
                if (composition_id) {
                    ...
                    console.log(`[getStudentBulletin] ID étudiant demandé: ${id}`);
                    console.log(`[getStudentBulletin] ID étudiant dans le token (parent): ${req.user.student_id}`);
                    ...
                    console.log(`[getStudentBulletin] Toutes les notes pour étudiant ${id}, composition ${composition_id}:`, allGradesCheck.map(g => ({
                        grade_id: g.grade_id,
                    ...
                    // Vérification STRICTE : toutes les notes doivent appartenir exactement à cet étudiant
                    ...
                    if (wrongStudentGrades.length > 0) {
                        console.error(`[getStudentBulletin] ⚠️ ERREUR CRITIQUE: ${wrongStudentGrades.length} note(s) ne correspondent pas à l'étudiant ${id}:`, wrongStudentGrades);
                    ...
                    console.log(`[getStudentBulletin] Détails complets des notes depuis grades pour l'étudiant ${id}:`, compositionGrades.map(g => ({ 
                        grade_id: g.grade_id,
                        subject_name: g.subject_name, 
                    ...
                    try {
                        ...
                        if (!student.class_id) {
                            ...
                            throw new Error('class_id manquant pour l\'étudiant');
                        }
                ...
                for (const subject of subjectGrades) {
                    ...
                    // Récupérer tous les étudiants de la classe pour cette matière
        ...
        // Récupérer les compositions disponibles pour un étudiant avec leur statut de publication
        getStudentCompositions: async(req, res) => {
            try {
                ...
                // Vérifier que l'utilisateur a le droit de voir les compositions de cet étudiant
                if (req.user.role === 'parent') {
                    const tokenStudentId = req.user.student_id;
                    if (!tokenStudentId || Number(tokenStudentId) !== Number(id)) {
                        console.log(`[getStudentCompositions] Accès refusé - token student_id: ${tokenStudentId}, demandé: ${id}`);
                        return res.status(403).json({
                            message: 'Vous n\'avez pas l\'autorisation de voir les compositions de cet étudiant'
                        });
                    }
                ...
                if (studentInfo.length === 0) {
                    return res.status(404).json({ message: 'Étudiant non trouvé ou non inscrit pour cette année' });
                }
                ...
                <body>
                    ...
                    <div class="section">
                        ...
                        <div class="photo-container">
                            ${data.child_photo ? `
                            <img class="student-photo" src="https://trayebernard-primaire.com/api/students/photo/${encodeURIComponent(data.child_photo)}" alt="Photo de ${data.first_name} ${data.last_name}">
                            ` : `
