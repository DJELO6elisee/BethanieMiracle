...
const Receipt = ({ data, onClose, receiptRef, handleDownload }: {
  ...
  const handlePrint = () => {
    if (receiptRef.current) {
      ...
      if (printWindow) {
        printWindow.document.write('<html><head><title>Reçu d\'inscription - GS BETHANIE MIRACLE</title>');
...
const InscrptionPre = ({ onClose, initialData }: { onClose: () => void, initialData?: any }) => {
  ...
  useEffect(() => {
    ...
    axios.get('https://bethaniemiracle.com/api/classes', {
  ...
  const handleSubmit = async () => {
    try {
      ...
      await axios.post(
        'https://bethaniemiracle.com/api/students',
        form,
        {
          headers: {
        ...
        .then(async (response) => {
          console.log('✅ Réponse de création d\'étudiant:', response.data);
          const now = new Date();
          
          // Récupère les infos complètes de l'élève nouvellement créé
          const { data: studentData } = await axios.get(
            `https://bethaniemiracle.com/api/students/${response.data.id}`,
            {
              headers: {
                Authorization: `Bearer ${localStorage.getItem('token')}`
              }
          ...
          console.log('✅ Données complètes de l\'étudiant:', studentData);
