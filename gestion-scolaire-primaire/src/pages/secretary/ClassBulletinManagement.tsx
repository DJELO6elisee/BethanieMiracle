...
const ClassBulletinManagement: React.FC = () => {
  ...
  const fetchClassData = async () => {
    try {
      ...
      const response = await axios.get(`https://bethaniemiracle.com/api/bulletins/class/${classId}/status`, {
  ...
  const fetchStudents = async () => {
    try {
      ...
      const response = await axios.get(`https://bethaniemiracle.com/api/bulletins/class/${classId}/students?school_year=${currentSchoolYear}`, {
        headers: { Authorization: `Bearer ${token}` }
      });

      console.log(`[ClassBulletinManagement] Étudiants récupérés: ${response.data.students?.length || 0} (total_in_enrollments: ${response.data.total_in_enrollments || 0})`);
  ...
  const confirmAction = async () => {
    ...
    try {
      ...
      await axios.post(`https://bethaniemiracle.com/api/bulletins/class/${classId}/${endpoint}`, payload, {
        headers: { Authorization: `Bearer ${token}` }
      });
  ...
  // Filtrer les étudiants par nom et matricule
