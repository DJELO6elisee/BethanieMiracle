...
const Subjects = () => {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      if (isEditMode && selectedSubject) {
        await axios.put(`https://bethaniemiracle.com/api/subjects/${selectedSubject.id}`, formData, config);
      } else {
        await axios.post('https://bethaniemiracle.com/api/subjects', formData, config);
      }
  ...
  const handleDeleteSubject = async (subjectId: number) => {
    if (window.confirm('Voulez-vous vraiment supprimer cette matière ? Cette action est irréversible.')) {
      try {
        const token = localStorage.getItem('token');
        await axios.delete(`https://bethaniemiracle.com/api/subjects/${subjectId}`, {
          headers: { Authorization: `Bearer ${token}` }
        });
        fetchSubjects();
      } catch (err: any) {
