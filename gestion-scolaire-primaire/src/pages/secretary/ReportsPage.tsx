...
const ReportsPage: React.FC = () => {
  ...
  const fetchClasses = async () => {
    try {
      ...
      const response = await axios.get('https://bethaniemiracle.com/api/classes', {
  ...
  const fetchCompositions = async (classId: number) => {
    try {
      const token = localStorage.getItem('token');
      const response = await axios.get(`https://bethaniemiracle.com/api/reports/compositions/${classId}`, {
        headers: { Authorization: `Bearer ${token}` }
      });
  ...
  const fetchAllCompositions = async () => {
    try {
      const token = localStorage.getItem('token');
      const response = await axios.get('https://bethaniemiracle.com/api/reports/compositions', {
        headers: { Authorization: `Bearer ${token}` }
      });
      setCompositions(response.data);
    } catch (error) {
  ...
  const generateReport = async () => {
    ...
    try {
      ...
      const url = selectedScope === 'class' 
        ? `https://bethaniemiracle.com/api/reports/composition/${selectedCompositionId}/class/${selectedClassId}`
        : `https://bethaniemiracle.com/api/reports/composition/${selectedCompositionId}/school`;
  ...
  const exportToExcel = () => {
    ...
    groupedResults.forEach((group) => {
      ...
      sheetData.push(['EPP /EPV : BETHANIE MIRACLE', null, null, 'SECTEUR: AGBAN']);
  ...
  const exportToWord = () => {
    ...
    htmlContent += `
          ...
          <div style="margin-top: 30px;">
            <h3>Statistiques:</h3>
            <p>Nombre total d'étudiants: ${reportData.statistics.total_students}</p>
