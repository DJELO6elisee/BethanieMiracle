...
const BulletinClassManagement: React.FC = () => {
  ...
  const fetchStudentsByClass = async () => {
    try {
      ...
      // Récupérer les étudiants de la classe via la route spécifique
      ...
      const response = await axios.get(`https://bethaniemiracle.com/api/classes/${classId}/students?school_year=${currentSchoolYear}`, {
        headers: {
          Authorization: `Bearer ${token}`
        }
      });

      console.log(`[BulletinClassManagement] Récupération des étudiants pour la classe ${classId}, année ${currentSchoolYear}`);
      console.log(`[BulletinClassManagement] Réponse:`, response.data);

      // Si aucun étudiant trouvé avec l'année scolaire, essayer sans filtre d'année
      if (!response.data || response.data.length === 0) {
        console.log(`[BulletinClassManagement] Aucun étudiant trouvé avec l'année ${currentSchoolYear}, essai sans filtre d'année`);
        const fallbackResponse = await axios.get(`https://bethaniemiracle.com/api/classes/${classId}/students`, {
          headers: {
            Authorization: `Bearer ${token}`
          }
        });
  ...
  const fetchPublicationStatus = async () => {
    try {
      ...
      const currentSchoolYear = getCurrentSchoolYear();
      const response = await axios.get(`https://bethaniemiracle.com/api/bulletins/class/${classId}/status?school_year=${currentSchoolYear}`, {
  ...
  const handleStudentBulletinClick = (studentId: number, studentName: string) => {
    // Navigation vers la page de bulletin de l'étudiant
  ...
  const handleStudentNotesClick = (studentId: number, studentName: string, event: React.MouseEvent) => {
    ...
    // Navigation vers la gestion des notes pour cet étudiant
    // On utilise la même logique que pour le bouton "Voir les notes" de classe
    // mais on pré-sélectionne l'étudiant spécifique
    navigate(`/teacher/grade-management/${classId}`, {
      state: { 
        ...
        preSelectedStudentId: studentId, // Pré-sélectionner l'étudiant
