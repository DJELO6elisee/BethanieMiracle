...
const MediaManagementPage = () => {
  ...
  const fetchMedia = async () => {
    try {
      ...
      const response = await axios.get('https://bethaniemiracle.com/api/media/admin/all', {
  ...
  const fetchStudents = async () => {
    try {
      const token = localStorage.getItem('token');
      const response = await axios.get('https://bethaniemiracle.com/api/students?include_all=true', {
        headers: { Authorization: `Bearer ${token}` }
      });
  ...
  const handleUpload = async () => {
    ...
    try {
      ...
      const token = localStorage.getItem('token');
      await axios.post('https://bethaniemiracle.com/api/media/upload', formData, {
        headers: {
  ...
  const handleDelete = async (mediaId: number) => {
    ...
    try {
      const token = localStorage.getItem('token');
      await axios.delete(`https://bethaniemiracle.com/api/media/${mediaId}`, {
        headers: { Authorization: `Bearer ${token}` }
      });
  ...
  const handleSendToParents = async (mediaId: number) => {
    try {
      const token = localStorage.getItem('token');
      await axios.post(`https://bethaniemiracle.com/api/media/${mediaId}/send-to-parents`, {}, {
        headers: { Authorization: `Bearer ${token}` }
      });
  ...
  const getMediaUrl = (media: Media) => {
    const token = localStorage.getItem('token');
    return `https://bethaniemiracle.com/api/media/${media.id}?token=${token}`;
  };
