...
const Teachers = () => {
  ...
  const fetchTeachers = useCallback(async () => {
    ...
    try {
      ...
      const res = await axios.get('https://bethaniemiracle.com/api/teachers', {
  ...
  const fetchSubjects = useCallback(async () => {
    try {
      const token = localStorage.getItem('token');
      const res = await axios.get('https://bethaniemiracle.com/api/subjects', {
        headers: { Authorization: `Bearer ${token}` }
      });
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      if (editModalOpen && selectedTeacher) {
        await axios.put(`https://bethaniemiracle.com/api/teachers/${selectedTeacher.id}`, dataToSend, {
          headers: { Authorization: `Bearer ${token}` }
        });
      } else {
        await axios.post('https://bethaniemiracle.com/api/teachers', dataToSend, {
          headers: { Authorization: `Bearer ${token}` }
        });
      }
  ...
  const handleDeleteTeacher = async (teacherId: number) => {
    if (window.confirm('Voulez-vous vraiment supprimer ce professeur ?')) {
      try {
        const token = localStorage.getItem('token');
        await axios.delete(`https://bethaniemiracle.com/api/teachers/${teacherId}`, {
          headers: { Authorization: `Bearer ${token}` }
        });
        fetchTeachers();
      } catch (err) {
  ...
  const openViewTeacher = useCallback(async (teacher: any) => {
    ...
    try {
      ...
      const [resTeacher, resAbsences] = await Promise.all([
        axios.get(`https://bethaniemiracle.com/api/teachers/${teacher.id}`, { headers: { Authorization: `Bearer ${token}` } }),
        axios.get(`https://bethaniemiracle.com/api/teacher-absences/teacher/${teacher.id}`, { headers: { Authorization: `Bearer ${token}` } })
      ]);
  ...
  const fetchTeacherAbsences = useCallback(async (teacherId: number) => {
    try {
      const token = localStorage.getItem('token');
      const res = await axios.get(`https://bethaniemiracle.com/api/teacher-absences/teacher/${teacherId}` , {
        headers: { Authorization: `Bearer ${token}` }
      });
  ...
  const handleCreateTeacherAbsence = async () => {
    ...
    try {
      ...
      await axios.post('https://bethaniemiracle.com/api/teacher-absences', {
  ...
  const handleDeleteTeacherAbsence = async (id: number) => {
    ...
    try {
      const token = localStorage.getItem('token');
      await axios.delete(`https://bethaniemiracle.com/api/teacher-absences/${id}`, {
        headers: { Authorization: `Bearer ${token}` }
      });
