...
const StudentInstallments: React.FC = () => {
    ...
    const fetchStudents = async () => {
        try {
            setLoading(true);
            const response = await axios.get('https://bethaniemiracle.com/api/students', {
    ...
    const fetchStudentInstallments = async (studentId: number) => {
        try {
            const response = await axios.get(`https://bethaniemiracle.com/api/installments/student/${studentId}`, {
                headers: { Authorization: `Bearer ${localStorage.getItem('token')}` },
                params: { school_year: selectedSchoolYear }
            });
    ...
    const fetchInstallmentPayments = async (installmentId: number) => {
        try {
            const response = await axios.get(`https://bethaniemiracle.com/api/installments/${installmentId}/payments`, {
                headers: { Authorization: `Bearer ${localStorage.getItem('token')}` }
            });
    ...
    const fetchOverdueStudents = async (filter: 'all' | 'level' | 'class', id?: number) => {
        try {
            ...
            const response = await axios.get(`https://bethaniemiracle.com/api${endpoint}`, {
                headers: { Authorization: `Bearer ${localStorage.getItem('token')}` },
                params: { school_year: selectedSchoolYear }
            });
            ...
            if (filter === 'all') {
                ...
                const detailedResponse = await axios.get('https://bethaniemiracle.com/api/reminders/all-school/overdue', {
                    headers: { Authorization: `Bearer ${localStorage.getItem('token')}` },
                    params: { school_year: selectedSchoolYear }
                });
