...
const Cantine: React.FC = () => {
    ...
    const loadCantineStudents = async () => {
        try {
            ...
            const response = await axios.get(`https://bethaniemiracle.com/api/cantine/students?school_year=${schoolYear}&month_number=${selectedMonth}&year=${selectedYear}`, { headers });
    ...
    const loadTrancheInfo = async (studentId: number) => {
        try {
            ...
            const response = await axios.get(`https://bethaniemiracle.com/api/cantine/tranche-info?student_id=${studentId}&school_year=${schoolYear}`, { headers });
    ...
    const getGarderiePayments = async (studentId: number) => {
        try {
            ...
            const response = await axios.get(`https://bethaniemiracle.com/api/cantine/student/${studentId}/payments?school_year=${schoolYear}`, { headers });
    ...
    const handlePayment = async () => {
        ...
        try {
            ...
            const response = await axios.post('https://bethaniemiracle.com/api/cantine/payment', {
    ...
    const fetchReceiptHistory = async (studentId: number) => {
        ...
        try {
            const token = localStorage.getItem('token');
            const response = await fetch(`https://bethaniemiracle.com/api/cantine/${studentId}/receipt-history?school_year=${schoolYear}`, {
                headers: {
    ...
    const handlePrintReceiptFromHistory = async (receipt: any) => {
        try {
            const token = localStorage.getItem('token');
            const url = `https://bethaniemiracle.com/api/cantine/${selectedStudentForHistory?.id}/payment-receipt/${receipt.payment_id}?school_year=${schoolYear}`;
                ...
                <Paper sx={{ p: 2, borderRadius: 2, boxShadow: 2 }}>
                    ...
                    <div style={{ display: 'none' }} ref={printPaidRef}>
                        <div className="header">
                            ...
                            <h1 style={{ color: '#2e7d32', marginBottom: '10px' }}>GS BÉTHANIE MIRACLE</h1>
                                    ...
                                    <TableRow key={student.id} sx={{ '&:hover': { backgroundColor: theme.palette.action.hover } }}>
                                        <TableCell>
                                            <Box sx={{ display: 'flex', alignItems: 'center', gap: 1 }}>
                                                {student.child_photo ? (
                                                    <Avatar
                                                        src={`https://bethaniemiracle.com/api/students/photo/${student.child_photo}`}
                                            ...
                                            <Box sx={{ display: 'flex', alignItems: 'center' }}>
                                                {student.source_type === 'garderie' ? (
                                                    ...
                                                    (() => {
                                                        ...
                                                        return !isFullyPaid && userRole !== 'secretary' ? (
                                                            <IconButton
                                                                color="primary"
                                                                size="small"
                                                                onClick={async () => {
                                            ...
                                            <Grid item xs={12} md={6}>
                                                <Box sx={{ display: 'flex', alignItems: 'center', gap: 2, mb: 2 }}>
                                                    {selectedStudentForDetails.child_photo ? (
                                                        <Avatar
                                                            src={`https://bethaniemiracle.com/api/students/photo/${selectedStudentForDetails.child_photo}`}
