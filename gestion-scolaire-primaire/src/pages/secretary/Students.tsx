...
const Students = () => {
  ...
  const editStudentPhotoUrl = editPhotoPreview || (editStudent?.child_photo ? `https://bethaniemiracle.com/api/students/photo/${editStudent.child_photo}` : null);
  ...
  const fetchStudents = async () => {
    try {
      ...
      const { data } = await axios.get(`https://bethaniemiracle.com/api/students?${params}`, {
  ...
  const fetchClasses = async (isMounted?: boolean) => {
    ...
    try {
      const token = localStorage.getItem('token');
      const res = await axios.get('https://bethaniemiracle.com/api/classes', {
        headers: { Authorization: `Bearer ${token}` }
      });
  ...
  const handleDelete = async (studentId: number) => {
    if (window.confirm('Voulez-vous vraiment supprimer cet élève ?')) {
      try {
        const token = localStorage.getItem('token');
        await axios.delete(`https://bethaniemiracle.com/api/students/${studentId}`, {
          headers: { Authorization: `Bearer ${token}` }
        });
        fetchStudents();
      } catch (err) {
  ...
  const handleEditSubmit = async () => {
    ...
    try {
      ...
      const url = `https://bethaniemiracle.com/api/students/${editStudent.id}`;
  ...
  const handlePaymentSubmit = async () => {
    ...
    try {
      ...
      const { data } = await axios.post(`https://bethaniemiracle.com/api/payments`, {
        student_id: studentToPay.id,
        amount: Number(paymentAmount),
        school_year: schoolYear,
        payment_method: 'cash',
  ...
  // Handler Finaliser
    ...
    try {
      ...
      setTimeout(() => {
        ...
        // Ajout des states pour les contacts WhatsApp lors de la finalisation
        console.log('=== [DEBUG] Étudiant dans handleFinalizeOpen:', student);
  ...
  const handleFinalizeSubmit = async () => {
    ...
    try {
      ...
      const { data } = await axios.post(`https://bethaniemiracle.com/api/students/${student.id}/finalize`, payload, {
        headers: { Authorization: `Bearer ${token}` }
      });
          ...
          <div class="receipt-container">
            ...
            <div class="section parent">
              ...
              <div class="info-grid">
                ...
                <div class="info-item">
                  <div class="info-label">Prénom du parent:</div>
  ...
  const handleMatriculeSearch = async () => {
    ...
    try {
      const token = localStorage.getItem('token');
      const { data } = await axios.get(`https://bethaniemiracle.com/api/students/search/registration/${matriculeSearch}`,
        { headers: { Authorization: `Bearer ${token}` } });
      ...
      if (data && data.found && data.student) {
        ...
        console.log('[DEBUG] Étudiant trouvé:', data.student.first_name, data.student.last_name);
  ...
  const handleReinscriptionSubmit = async () => {
    ...
    try {
      ...
      //   });
      //   
      //   await axios.post(`https://bethaniemiracle.com/api/payments`, {
      //     student_id: reinscriptionStudent.id,
      //     amount: previousYearPayment,
      //     school_year: getPreviousSchoolYear(),
      //     payment_method: 'cash',
      ...
      const { data } = await axios.post(`https://bethaniemiracle.com/api/students/${reinscriptionStudent.id}/reinscription`, {
  ...
  useEffect(() => {
    const fetchAnnualAverage = async () => {
      if (reinscriptionStudent) {
        try {
          const token = localStorage.getItem('token');
          const { data } = await axios.get(`https://bethaniemiracle.com/api/students/${reinscriptionStudent.id}/annual-average`, {
            headers: { Authorization: `Bearer ${token}` }
          });
          // Admis si moyenne >= 10
  ...
  const handleShowReceipt = async (paymentId: number | string) => {
    try {
      const token = localStorage.getItem('token');
      const { data } = await axios.get(`https://bethaniemiracle.com/api/payments/${paymentId}/receipt`, {
        headers: { Authorization: `Bearer ${token}` }
      });
  ...
  const fetchReceiptHistory = async (studentId: number) => {
    ...
    try {
      const token = localStorage.getItem('token');
      const { data } = await axios.get(`https://bethaniemiracle.com/api/students/${studentId}/receipt-history`, {
        headers: { Authorization: `Bearer ${token}` }
      });
  ...
  const handlePrintReceiptFromHistory = async (receipt: any) => {
    try {
      ...
      switch (receipt.type) {
        case 'inscription':
          const inscriptionResponse = await axios.get(`https://bethaniemiracle.com/api/students/${receipt.student_id}/inscription-receipt?enrollment_date=${receipt.created_at}`, {
            headers: { Authorization: `Bearer ${token}` }
          });
        ...
        case 'paiement':
          ...
          const paymentResponse = await axios.get(`https://bethaniemiracle.com/api/students/${receipt.student_id}/payment-receipt/${receipt.payment_id}`, {
            headers: { Authorization: `Bearer ${token}` }
          });
                                  ...
                                  <TableCell align="right">
                                    {isToFinalize ? (
                                      ...
                                      <Box sx={{ display: 'flex', justifyContent: 'flex-end', gap: 0.5 }}>
                                        {remaining > 0 && userRole !== 'secretary' &&
                      ...
                      <Box ref={paymentReceiptRef} sx={{ p: 4, border: '1px solid #ddd', borderRadius: '8px', bgcolor: '#fff' }}>
                        ...
                        {paymentReceiptData.child_photo && (
                          <Box sx={{ display: 'flex', justifyContent: 'center', mb: 3 }}>
                            <Box
                              component="img"
                              src={`https://bethaniemiracle.com/api/students/photo/${encodeURIComponent(paymentReceiptData.child_photo)}`}
                    ...
                    <div style={{ padding: '30px' }}>
                      ...
                      <div style={{
                        ...
                        <Grid container spacing={3}>
                          ...
                          <Grid item xs={12} sm={6}>
                            <div style={{ marginBottom: '15px' }}>
                              <strong style={{ color: '#555' }}>Prénom du parent:</strong>
                              <div style={{ fontSize: '16px', fontWeight: 'bold', color: '#4caf50' }}>
                  ...
                  {reinscriptionReceiptData && (
                    <Box ref={reinscriptionReceiptRef} sx={{ p: 4, border: '1px solid #1976d2', borderRadius: '16px', bgcolor: '#fafdff', boxShadow: 4, maxWidth: 700, mx: 'auto', my: 2 }}>
                      ...
                      <Grid container spacing={1}>
                        ...
                        <Grid item xs={12} sm={6}><Typography><b>Prénom du parent:</b>{reinscriptionReceiptData.parent.parent_last_name}</Typography></Grid>
