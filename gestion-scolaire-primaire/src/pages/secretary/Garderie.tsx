...
const Garderie = () => {
  ...
  const fetchChildren = async () => {
    setLoading(true);
    try {
      const response = await fetch(`https://bethaniemiracle.com/api/garderie/children?schoolYear=${selectedSchoolYear}`, {
        method: 'GET',
        headers: {
        ...
        if (data.children && data.children.length > 0) {
          data.children.forEach((child: any, index: number) => {
            console.log(`👶 Enfant ${index + 1}:`, {
              ...
              photoUrl: child.child_photo ? `https://bethaniemiracle.com/api/garderie/photo/${child.child_photo}` : 'Aucune photo'
            });
          });
        }
  ...
  const handleSubmit = async () => {
    try {
      ...
      try {
        ...
        saveResponse = await fetch('https://bethaniemiracle.com/api/garderie/save-inscription', {
          method: 'POST',
          headers: {
            'Authorization': `Bearer ${localStorage.getItem('token')}`
          },
  ...
  const searchChild = async (searchTerm: string) => {
    ...
    try {
      const response = await fetch(`https://bethaniemiracle.com/api/garderie/search?term=${encodeURIComponent(searchTerm)}`, {
        method: 'GET',
        headers: {
          'Authorization': `Bearer ${localStorage.getItem('token')}`
        }
  ...
  const handleReinscriptionSubmit = async () => {
    try {
      ...
      try {
        ...
        saveResponse = await fetch('https://bethaniemiracle.com/api/garderie/reinscription', {
          method: 'POST',
          headers: {
            'Authorization': `Bearer ${localStorage.getItem('token')}`
          },
  ...
  const confirmDelete = async () => {
    ...
    try {
      const response = await fetch(`https://bethaniemiracle.com/api/garderie/inscription/${deletingChild.id}`, {
        method: 'DELETE',
        headers: {
  ...
  const handleEditSubmit = async () => {
    try {
      ...
      const response = await fetch(`https://bethaniemiracle.com/api/garderie/inscription/${editingChild.id}`, {
        method: 'PUT',
        headers: {
          'Authorization': `Bearer ${localStorage.getItem('token')}`
        },
  ...
  const handleMakePayment = async () => {
    ...
    try {
      const response = await fetch('https://bethaniemiracle.com/api/garderie/payment', {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
          'Authorization': `Bearer ${localStorage.getItem('token')}`
  ...
  const fetchReceiptHistory = async (childId: number) => {
    ...
    try {
      const token = localStorage.getItem('token');
      const response = await fetch(`https://bethaniemiracle.com/api/garderie/${childId}/receipt-history`, {
        headers: {
  ...
  const handlePrintReceiptFromHistory = async (receipt: any) => {
    try {
      ...
      if (receipt.type === 'inscription') {
        url = `https://bethaniemiracle.com/api/garderie/${selectedChildForHistory.id}/inscription-receipt?inscription_date=${receipt.date}`;
      } else if (receipt.type === 'paiement') {
        url = `https://bethaniemiracle.com/api/garderie/${selectedChildForHistory.id}/payment-receipt/${receipt.payment_id}`;
      }
  ...
  const handlePrintChildrenList = () => {
    ...
    if (printWindow) {
      ...
      printWindow.document.write('<div class="school-name">LA PETITE ACADEMIE</div>');
      printWindow.document.write('<div class="school-subtitle">LA MAISON DES ENFANTS</div>');
  ...
  const testPhotoAccess = async () => {
    try {
      ...
      const testUrl = `https://bethaniemiracle.com/api/garderie/photo/${testPhoto}`;
              ...
              <Grid container spacing={3}>
                ...
                <Grid item xs={12} sx={{ textAlign: 'center', mb: 2 }}>
                  {viewingChild.child_photo && !photoError[viewingChild.id] ? (
                    <img
                      src={`https://bethaniemiracle.com/api/garderie/photo/${viewingChild.child_photo}`}
                      ...
                      onError={(e) => {
                        ...
                        console.error(`🔗 URL complète: https://bethaniemiracle.com/api/garderie/photo/${viewingChild.child_photo}`);
                      ...
                      onLoad={() => {
                        ...
                        console.log(`📸 URL: https://bethaniemiracle.com/api/garderie/photo/${viewingChild.child_photo}`);
                      }}
                    />
                  ) : (
                    <Box
                    ...
                    <TableRow key={child.id || index} sx={{ '&:hover': { backgroundColor: 'rgba(25, 118, 210, 0.04)' } }}>
                      <TableCell>
                        <Box sx={{ display: 'flex', alignItems: 'center', gap: 2 }}>
                          {(() => {
                            ...
                            console.log(`🔗 URL: ${child.child_photo ? `https://bethaniemiracle.com/api/garderie/photo/${child.child_photo}` : 'Aucune photo'}`);
                            
                            return child.child_photo && !photoError[child.id] ? (
                              <img
                                src={`https://bethaniemiracle.com/api/garderie/photo/${child.child_photo}`}
                                ...
                                onError={(e) => {
                                  ...
                                  console.error(`🔗 URL complète: https://bethaniemiracle.com/api/garderie/photo/${child.child_photo}`);
                                  console.error(`📊 Événement d'erreur:`, e);
                      ...
                      <TableCell>
                        <Box sx={{ display: 'flex', gap: 1 }}>
                          ...
                          {child.total_schooling_amount && child.total_paid_amount !== undefined && 
                           (child.total_schooling_amount - child.total_paid_amount) > 0 && userRole !== 'secretary' && (
