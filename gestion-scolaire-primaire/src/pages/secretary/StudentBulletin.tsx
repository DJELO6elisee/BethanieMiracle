...
const StudentBulletin: React.FC = () => {
  ...
  // Charger les matières une fois que la classe est connue (étudiant chargé ou classe en state)
  useEffect(() => {
    if (!studentId) return;
  ...
  useEffect(() => {
    ...
    console.log('🎧 [STUDENT BULLETIN] Mise en place de l\'écoute des événements pour étudiant:', studentId);
    ...
    const unsubscribeGradeUpdate = gradeEventManager.onGradeUpdate((data) => {
      ...
      // Vérifier si cet événement concerne cet étudiant
      if (data.studentId === parseInt(studentId)) {
        console.log('✅ [STUDENT BULLETIN] Mise à jour concernant cet étudiant, rafraîchissement...');
    ...
    const unsubscribeBulletinRefresh = gradeEventManager.onBulletinRefresh((data) => {
      ...
      // Vérifier si cet étudiant est dans la liste des étudiants à rafraîchir
      if (data.studentIds.includes(parseInt(studentId))) {
        console.log('✅ [STUDENT BULLETIN] Rafraîchissement demandé pour cet étudiant');
  ...
  const fetchClassStudentsCount = async (studentData?: Student | null) => {
    ...
    try {
      ...
      // Récupérer l'ID de la classe depuis les données de l'étudiant
      ...
      // Récupérer les étudiants de la classe - essayer d'abord avec tous les étudiants
      let response;
      try {
        // Essayer d'abord avec un endpoint qui retourne tous les étudiants (sans filtre de statut)
        response = await axios.get(`https://bethaniemiracle.com/api/classes/${classId}/students?school_year=${currentSchoolYear}&include_all=true`, {
          headers: { Authorization: `Bearer ${token}` }
        });
      } catch (err: any) {
        // Si ça échoue, utiliser l'endpoint standard
        response = await axios.get(`https://bethaniemiracle.com/api/classes/${classId}/students?school_year=${currentSchoolYear}`, {
          headers: { Authorization: `Bearer ${token}` }
        });
      }

      console.log('🔍 [STUDENT BULLETIN] Réponse API étudiants de la classe:', response.data);
      
      const students = Array.isArray(response.data) ? response.data : response.data?.students || [];
      const count = students.length;
      
      // Si on a moins que prévu, essayer aussi de récupérer tous les étudiants de la classe sans filtre
      if (count < 43) {
        ...
        try {
          ...
          const allStudentsResp = await axios.get(`https://bethaniemiracle.com/api/students?class_id=${classId}&school_year=${currentSchoolYear}&include_all=true`, {
            headers: { Authorization: `Bearer ${token}` }
          });
          const allStudents = Array.isArray(allStudentsResp.data) ? allStudentsResp.data : allStudentsResp.data?.data || [];
          console.log('🔍 [STUDENT BULLETIN] Tous les étudiants de la classe (sans filtre):', allStudents.length);
          ...
          console.warn('⚠️ [STUDENT BULLETIN] Impossible de récupérer tous les étudiants:', err2);
        }
      }
      
      // Toujours mettre à jour l'effectif même s'il est différent
      console.log('📊 [STUDENT BULLETIN] Nombre d\'étudiants trouvés:', count, 'sur', students.length, 'total');
  ...
  const fetchStudentData = async () => {
    try {
      ...
      const currentSchoolYear = getCurrentSchoolYear();
      const response = await axios.get(`https://bethaniemiracle.com/api/students/${studentId}?school_year=${currentSchoolYear}`, {
        headers: {
      ...
      // Récupérer l'effectif de la classe une fois l'étudiant chargé
      if (studentData) {
  ...
  const fetchCompositions = async () => {
    ...
    try {
      const token = localStorage.getItem('token');
      const response = await axios.get(`https://bethaniemiracle.com/api/students/${studentId}/compositions?school_year=2025-2026`, {
        headers: { Authorization: `Bearer ${token}` }
      });
  ...
  /**
   * Calcule la moyenne d'un élève à partir de ses bulletin_grades (API).
   * Utilise la même logique que calculateTotalAndAverage pour cohérence entre toutes les classes.
   */
  const calculateStudentAverageFromBulletinGrades = (bulletinGrades: Array<{ subject_name?: string; average?: number }>, cls: string): number => {
    const label = cls.toUpperCase();
    const isCP = label.startsWith('CP');
    const isCE = label.startsWith('CE');
    const isCE2 = label.startsWith('CE2');
    const isCM1 = label.startsWith('CM1');
    const isCM2 = label.startsWith('CM2');

    if (bulletinGrades.length === 0) return 0;
    ...
    if (isCE) {
      ...
      bulletinGrades.forEach((bg: any) => {
        ...
        if (targeted.some(key => name.includes(key))) {
          ...
          totalPoints += raw;
          let maxScore = 10;
          if (name.includes('EXPLOITATION DE TEXTE') || name.includes('A.E.M') || name.includes('MATHEMATIQUE') || name.includes('MATHÉMATIQUE')) maxScore = 30;
          else if (name.includes('ORTHOGRAPHE') || name.includes('DICTEE') || name.includes('DICTÉE')) maxScore = 20;
          totalMax += maxScore;
        }
      });
      if (totalMax > 0) return (totalPoints / totalMax) * 10;
      return totalPoints / (isCE2 ? 17 : 11);
    }

    if (isCM1 || isCM2) {
      const excluded = ['LECTURE', 'ANGLAIS', 'EPS', 'E.P.S', 'CONDUITE'];
      const totalPoints = bulletinGrades.reduce((sum, bg) => {
        const name = (bg.subject_name || '').toUpperCase();
        if (excluded.some(ex => name.includes(ex))) return sum;
        return sum + (Number(bg.average) || 0);
      }, 0);
      if (isCM1) return totalPoints / 17;  // moyenne sur 10
      return totalPoints / 8.5;             // moyenne sur 20
    }

    // Autres classes: moyenne simple
    const total = bulletinGrades.reduce((s, bg) => s + (Number(bg.average) || 0), 0);
    return bulletinGrades.length > 0 ? total / bulletinGrades.length : 0;
  };
  ...
  const calculateStudentRank = async (studentAverage: number) => {
    ...
    try {
      ...
      if (selectedComposition) {
        ...
        try {
          const response = await axios.get(`https://bethaniemiracle.com/api/debug/class-composition-averages/${classId}`, {
            params: {
        ...
        if (classAverages.length === 0) {
          // Sinon, récupérer tous les étudiants et calculer leurs moyennes
          const studentsResp = await axios.get(`https://bethaniemiracle.com/api/classes/${classId}/students?school_year=${currentSchoolYear}`, {
            headers: { Authorization: `Bearer ${token}` }
          });
          ...
          const averagesPromises = studentsList.map(async (s: any) => {
            try {
              const studentApiUrl = `https://bethaniemiracle.com/api/debug/student-bulletin/${s.id}?trimester=1er trimestre&school_year=${currentSchoolYear}&composition_id=${selectedComposition}`;
              ...
              if (isCE) {
                ...
                const clsLabel = (student?.classe_name || student?.class_name || student?.classe || '').toUpperCase();
                const average = calculateStudentAverageFromBulletinGrades(bulletinGrades, clsLabel);
                return { student_id: s.id, average };
              }
            } catch (err) {
        ...
        // Récupérer tous les étudiants et calculer leurs moyennes générales
        try {
          const studentsResp = await axios.get(`https://bethaniemiracle.com/api/classes/${classId}/students?school_year=${currentSchoolYear}`, {
            headers: { Authorization: `Bearer ${token}` }
          });
          ...
          const averagesPromises = studentsList.map(async (s: any) => {
            try {
              const studentApiUrl = `https://bethaniemiracle.com/api/debug/student-bulletin/${s.id}?trimester=1er trimestre&school_year=${currentSchoolYear}`;
              const studentData = await axios.get(studentApiUrl, {
                headers: { Authorization: `Bearer ${token}` }
              });
          ...
          if (validAverages.length === 1 && validAverages[0].student_id === Number(studentId)) {
            ...
            if (rank > 0) {
              const highestAvg = validAverages[0].average.toFixed(2);
              const lowestAvg = validAverages[validAverages.length - 1].average.toFixed(2);
              const totalClassAvg = validAverages.reduce((sum, a) => sum + a.average, 0);
              const classAvg = (totalClassAvg / validAverages.length).toFixed(2);
              setBulletinData(prev => ({
                ...prev,
                rangGeneral: String(rank),
                moyennePlusElevee: highestAvg,
  ...
  const fetchSubjects = async () => {
    try {
      ...
      const allResp = await axios.get('https://bethaniemiracle.com/api/bulletin-subjects', {
        headers: { Authorization: `Bearer ${token}` }
      });
  ...
  const fetchStudentGrades = async () => {
    try {
      ...
      let apiUrl = `https://bethaniemiracle.com/api/debug/student-bulletin/${studentId}?trimester=${encodeURIComponent(trimester)}&school_year=${encodeURIComponent(schoolYear)}`;
      if (selectedComposition) {
                  ...
                  <TableRow>
                    <TableCell sx={{ fontWeight: 'bold', fontSize: '0.9rem', py: 0.5 }}>
                      {(() => {
                        const cls = (student?.classe_name || student?.class_name || student?.classe || '').toUpperCase();
                        if (cls.startsWith('CM1') || cls.startsWith('CM2')) {
                          return 'Total des matières principales';
                        }
                        return 'Total';
                      })()}
                    </TableCell>
