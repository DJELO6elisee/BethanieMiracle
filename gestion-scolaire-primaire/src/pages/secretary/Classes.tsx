...
const Classes = () => {
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      ...
      try {
        ...
        const classesRes = await axios.get('https://bethaniemiracle.com/api/classes', {
          headers: { Authorization: `Bearer ${token}` },
          params: { school_year: schoolYear }
        });
        
        // Récupérer les niveaux d'études
        const levelsRes = await axios.get('https://bethaniemiracle.com/api/education-levels', {
          headers: { Authorization: `Bearer ${token}` }
        });
  ...
  const handleAddClass = async () => {
    try {
      const token = localStorage.getItem('token');
      await axios.post('https://bethaniemiracle.com/api/classes', {
        name: newClass.name,
  ...
  const handleDelete = async (id: number | string) => {
    ...
    if (window.confirm('Voulez-vous vraiment supprimer cette classe ?')) {
      try {
        const token = localStorage.getItem('token');
        const response = await axios.delete(`https://bethaniemiracle.com/api/classes/${id}`, {
          headers: { Authorization: `Bearer ${token}` }
        });
  ...
  const handleEditSubmit = async () => {
    ...
    try {
      ...
      const response = await axios.put(`https://bethaniemiracle.com/api/classes/${editClass.id}`, {
        name: editClass.name,
                      ...
                      <List dense>
                        <ListItem>
                          ...
                          <ListItemText
                            ...
                            secondary={classe.professeurPrincipal || classe.teacher || 'Non assigné'}
                          />
                        </ListItem>
                        <ListItem>
                          <ListItemText
