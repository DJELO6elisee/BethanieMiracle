...
const PaymentReminders: React.FC = () => {
    ...
    const fetchStatistics = async () => {
        try {
            setLoading(true);
            const response = await axios.get('https://bethaniemiracle.com/api/reminders/statistics', {
    ...
    const fetchOverdueStudents = async () => {
        try {
            const response = await axios.get('https://bethaniemiracle.com/api/reminders/overdue/all-school', {
                headers: { Authorization: `Bearer ${localStorage.getItem('token')}` },
                params: { school_year: '2025-2026' }
            });
    ...
    const sendReminders = async (type: string, id?: number | null) => {
        try {
            ...
            switch (type) {
                case 'all':
                    endpoint = 'https://bethaniemiracle.com/api/reminders/all-school';
                    break;
                case 'level':
                    endpoint = `https://bethaniemiracle.com/api/reminders/level/${id}`;
                    break;
                case 'class':
                    endpoint = `https://bethaniemiracle.com/api/reminders/class/${id}`;
                    break;
                case 'student':
                    endpoint = `https://bethaniemiracle.com/api/reminders/student/${id}`;
                    break;
            }
