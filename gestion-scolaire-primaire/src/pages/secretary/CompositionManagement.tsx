...
const CompositionManagement: React.FC = () => {
  ...
  const fetchCompositions = async () => {
    try {
      ...
      const response = await axios.get('https://bethaniemiracle.com/api/compositions', {
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      if (editingComposition) {
        // Modification
        await axios.put(`https://bethaniemiracle.com/api/compositions/${editingComposition.id}`, compositionData, {
          headers: { Authorization: `Bearer ${token}` }
        });
        setSnackbar({
          open: true,
        ...
        // Création
        await axios.post('https://bethaniemiracle.com/api/compositions', compositionData, {
          headers: { Authorization: `Bearer ${token}` }
        });
        setSnackbar({
          open: true,
  ...
  const handleDelete = async (composition: Composition) => {
    ...
    try {
      const token = localStorage.getItem('token');
      await axios.delete(`https://bethaniemiracle.com/api/compositions/${composition.id}`, {
        headers: { Authorization: `Bearer ${token}` }
      });
  ...
  const handleToggleStatus = async (composition: Composition) => {
    try {
      const token = localStorage.getItem('token');
      await axios.put(`https://bethaniemiracle.com/api/compositions/toggle-status/${composition.id}`, {}, {
        headers: { Authorization: `Bearer ${token}` }
      });
