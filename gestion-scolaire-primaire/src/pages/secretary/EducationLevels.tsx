...
const EducationLevels: React.FC = () => {
    ...
    const fetchData = async () => {
        try {
            setLoading(true);
            const response = await axios.get('https://bethaniemiracle.com/api/education-levels', {
    ...
    const handleOpenDialog = async (level?: EducationLevel) => {
        if (level) {
            ...
            try {
                const response = await axios.get(`https://bethaniemiracle.com/api/education-levels/${level.id}`, {
                    headers: { Authorization: `Bearer ${localStorage.getItem('token')}` }
                });
    ...
    const handleViewInstallments = async (level: EducationLevel) => {
        try {
            ...
            const response = await axios.get(`https://bethaniemiracle.com/api/education-levels/${level.id}/installments`, {
                headers: { Authorization: `Bearer ${localStorage.getItem('token')}` }
            });
    ...
    const handleSubmit = async () => {
        try {
            ...
            if (editingLevel) {
                await axios.put(`https://bethaniemiracle.com/api/education-levels/${editingLevel.id}`, payload, {
                    headers: { Authorization: `Bearer ${localStorage.getItem('token')}` }
                });
                setSnackbar({
                    open: true,
                ...
                await axios.post('https://bethaniemiracle.com/api/education-levels', payload, {
                    headers: { Authorization: `Bearer ${localStorage.getItem('token')}` }
                });
                setSnackbar({
                    open: true,
    ...
    const handleDelete = async (id: number) => {
        if (window.confirm('Êtes-vous sûr de vouloir supprimer ce niveau d\'études ?')) {
            try {
                await axios.delete(`https://bethaniemiracle.com/api/education-levels/${id}`, {
                    headers: { Authorization: `Bearer ${localStorage.getItem('token')}` }
                });
                setSnackbar({
                    open: true,
    ...
    return (
        <Box sx={{ display: 'flex', minHeight: '100vh', backgroundColor: '#f5f7fa' }}>
            <SecretarySidebar />
            <Box component="main" sx={{ flexGrow: 1, ml: '250px' }}>
                                            ...
                                            <Box sx={{ 
                                                ...
                                                <Box display="flex" justifyContent="space-between" alignItems="center" mb={1.5}>
                                                    <Box display="flex" alignItems="center" gap={1}>
                                                        <MoneyIcon sx={{ color: '#27ae60', fontSize: 20 }} />
                                                        <Typography variant="body2" color="text.secondary">Scolarité:</Typography>
                                                    </Box>
                                                    <Typography variant="body2" fontWeight="bold" sx={{ color: '#27ae60' }}>
                                                        {formatCurrency(level.tuition_amount)}
                                                    </Typography>
                                                </Box>
                                                <Box display="flex" justifyContent="space-between" alignItems="center" mb={1.5}>
                                                    <Box display="flex" alignItems="center" gap={1}>
                                                        <CheckCircleIcon sx={{ color: '#3498db', fontSize: 20 }} />
                                                        <Typography variant="body2" color="text.secondary">Inscription:</Typography>
                                                    </Box>
                                                    <Typography variant="body2" fontWeight="bold" sx={{ color: '#3498db' }}>
                                                ...
                                                <Box display="flex" justifyContent="space-between" alignItems="center">
                                                    ...
                                                    <Typography variant="body2" fontWeight="bold" sx={{ color: '#e74c3c' }}>
