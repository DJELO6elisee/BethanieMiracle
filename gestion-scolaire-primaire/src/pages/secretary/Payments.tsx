...
const Payments = () => {
  ...
  useEffect(() => {
    ...
    if (token) {
      try {
        ...
        if (['admin', 'secretary', 'comptable', 'informaticien'].includes(payload.role)) {
  ...
  useEffect(() => {
    ...
    const fetchFinancialSummary = async () => {
      ...
      try {
        ...
        const token = localStorage.getItem('token');
        const response = await axios.get(`https://bethaniemiracle.com/api/payments/financial-summary?school_year=${schoolYear}`, {
          headers: {
  ...
  useEffect(() => {
    ...
    const fetchDailyStates = async () => {
      ...
      try {
        const token = localStorage.getItem('token');
        const response = await axios.get(`https://bethaniemiracle.com/api/payments/daily-states?school_year=${schoolYear}`, {
          headers: {
            Authorization: `Bearer ${token}`
          }
        });
  ...
  const fetchCustomDateRangeStates = async (start: string, end: string) => {
    ...
    try {
      const token = localStorage.getItem('token');
      const response = await axios.get(`https://bethaniemiracle.com/api/payments/custom-date-range?start_date=${start}&end_date=${end}&school_year=${schoolYear}`, {
        headers: {
          Authorization: `Bearer ${token}`
        }
      });
