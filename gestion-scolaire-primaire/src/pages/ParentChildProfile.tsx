  ...
  useEffect(() => {
    ...
    const fetchPaymentSummary = async () => {
      ...
      try {
        ...
        const { data } = await axios.get(`https://bethaniemiracle.com/api/students/${childId}/payment-summary?school_year=${schoolYear}`, {
...
const PhotoVideoTab = ({ childId, schoolYear, user }: { childId: string | undefined, schoolYear: string, user: any }) => {
  ...
  useEffect(() => {
    ...
    const fetchMedia = async () => {
      ...
      try {
        const token = localStorage.getItem('token');
        const { data } = await axios.get(`https://bethaniemiracle.com/api/students/${childId}/media`, {
          headers: { Authorization: `Bearer ${token}` }
        });
...
const ParentChildProfile = () => {
  ...
  const markNotificationAsRead = async (notificationId: number) => {
    try {
      ...
      await axios.post('https://bethaniemiracle.com/api/parent/notification/read', {
        userId: user.id,
        notificationId
      });
      if (isMounted) {
  ...
  const markAllNotificationsAsRead = async () => {
    try {
      const user = JSON.parse(localStorage.getItem('user') || '{}');
      await axios.post('https://bethaniemiracle.com/api/parent/notification/read-all', {
        userId: user.id
      });
      if (isMounted) {
  ...
  const refreshNotifications = async () => {
    ...
    try {
      const user = JSON.parse(localStorage.getItem('user') || '{}');
      const token = localStorage.getItem('token');
      const { data } = await axios.get(`https://bethaniemiracle.com/api/events/my-notifications`, {
        headers: { Authorization: `Bearer ${token}` }
      });
      if (isMounted) {
  ...
  React.useEffect(() => {
    ...
    const fetchChildNotifications = async () => {
      ...
      try {
        const url = `https://bethaniemiracle.com/api/events/my-notifications`;
  ...
  React.useEffect(() => {
    ...
    const fetchAnnualAverage = async () => {
      ...
      try {
        const token = localStorage.getItem('token');
        const { data } = await axios.get(`https://bethaniemiracle.com/api/students/${childId}/annual-average?school_year=${schoolYear}`, {
          headers: { Authorization: `Bearer ${token}` }
        });
        if (isMounted) {
  ...
  React.useEffect(() => {
    ...
    const fetchClassId = async () => {
      ...
      try {
        const token = localStorage.getItem('token');
        const { data } = await axios.get(`https://bethaniemiracle.com/api/students/${childId}?school_year=${schoolYear}`, {
          headers: { Authorization: `Bearer ${token}` }
        });
        if (isMounted) {
  ...
  React.useEffect(() => {
    ...
    const fetchPublished = async () => {
      ...
      await Promise.all(trimesters.map(async (trimester) => {
        ...
        try {
          const { data } = await axios.get(`https://bethaniemiracle.com/api/report-cards/published?class_id=${studentClassId}&trimester=${encodeURIComponent(trimester)}&school_year=${schoolYear}`, {
            headers: { Authorization: `Bearer ${token}` }
          });
          if (isMounted) {
