...
const TeacherDashboard = () => {
  ...
  const handleSelectClass = async (classe: Class) => {
    ...
    try {
      ...
      const absencesPromise = axios.get(`https://bethaniemiracle.com/api/absences?class_id=${classe.id}&subject_id=${selectedSubject}`, { 
        headers: { Authorization: `Bearer ${token}` }
      });
  ...
  const handleSubmitAbsences = async () => {
    ...
    try {
      ...
      await axios.post('https://bethaniemiracle.com/api/absences/bulk', absencesToSubmit, {
        headers: { Authorization: `Bearer ${token}` }
      });
