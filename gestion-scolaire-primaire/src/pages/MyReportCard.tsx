...
const MyReportCard = () => {
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      ...
      try {
        // 1. Charger l'élève d'abord
        const studentRes = await axios.get('https://bethaniemiracle.com/api/students/me', {
        ...
        let pubRes;
        try {
          pubRes = await axios.get('https://bethaniemiracle.com/api/report-cards/published', {
            params: {
              class_id,
              trimester,
              school_year
        ...
        // 4. Récupérer toutes les notes de l'élève
        const gradesRes = await axios.get(`https://bethaniemiracle.com/api/students/${studentRes.data.student.id}/grades`, {
          headers: { Authorization: `Bearer ${token}` }
        });
        if (isMounted) {
        ...
        if (typeof trimester === 'string') {
          const rankRes = await axios.get(
            `https://bethaniemiracle.com/api/students/${studentRes.data.student.id}/trimester-rank?semester=${encodeURIComponent(trimester)}`,
            { headers: { Authorization: `Bearer ${token}` } }
          );
          if (isMounted) {
      ...
      <Paper ref={bulletinRef} sx={{ p: 4, borderRadius: 5, maxWidth: 900, mx: 'auto', boxShadow: '0 8px 32px rgba(80, 36, 204, 0.10)', background: 'white' }}>
        ...
        <Stack direction="row" spacing={4} sx={{ mb: 2 }}>
          <Box>
            ...
            <Typography>
              <b>Date de naissance :</b> {student?.date_of_birth ? (() => {
                try {
