...
const BulletinTab = ({ childId, schoolYear }: { childId: string | undefined, schoolYear: string }) => {
  ...
  useEffect(() => {
    ...
    console.log('🎧 [BULLETIN TAB] Mise en place de l\'écoute des événements pour étudiant:', childId);
    ...
    const unsubscribeGradeUpdate = gradeEventManager.onGradeUpdate((data) => {
      ...
      if (data.studentId === parseInt(childId)) {
        console.log('✅ [BULLETIN TAB] Mise à jour concernant cet étudiant, rafraîchissement...');
    ...
    const unsubscribeBulletinRefresh = gradeEventManager.onBulletinRefresh((data) => {
      ...
      if (data.studentIds.includes(parseInt(childId))) {
        console.log('✅ [BULLETIN TAB] Rafraîchissement demandé pour cet étudiant');
  ...
  const fetchStudentData = async () => {
    try {
      ...
      const response = await axios.get(`https://bethaniemiracle.com/api/students/${childId}?school_year=${schoolYear}`, {
        headers: {
  ...
  const fetchCompositions = async () => {
    ...
    try {
      const response = await axios.get(`https://bethaniemiracle.com/api/students/${childId}/compositions?school_year=${schoolYear}`, {
  ...
  const fetchStudentGrades = async () => {
    try {
      ...
      let url = `https://bethaniemiracle.com/api/students/${childId}/bulletin?school_year=${schoolYear}`;
