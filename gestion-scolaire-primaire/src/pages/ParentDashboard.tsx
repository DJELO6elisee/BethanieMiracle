  ...
  useEffect(() => {
    ...
    const fetchChildren = async () => {
      ...
      try {
        ...
        const { data } = await axios.get(`https://bethaniemiracle.com/api/students`, {
    ...
    const fetchNotifications = async () => {
      ...
      if (!user.id) return;
      try {
        const { data } = await axios.get(`https://bethaniemiracle.com/api/parent/${user.id}/notifications`, {
          headers: { Authorization: `Bearer ${token}` }
        });
        if (isMounted) {
    ...
    const fetchParent = async () => {
      ...
      const token = localStorage.getItem('token');
      try {
        const { data } = await axios.get(`https://bethaniemiracle.com/api/auth/me?school_year=${schoolYear}`, {
          headers: { Authorization: `Bearer ${token}` }
        });
        if (isMounted) {
          setParent(data);
