...
const GradeManagement: React.FC = () => {
  ...
  const isAdminAccess = location.state?.isAdminAccess || (userRole === 'admin' || userRole === 'secretary' || userRole === 'informaticien');
  ...
  const fetchClassInfo = async () => {
    ...
    try {
      ...
      const response = await axios.get(`https://bethaniemiracle.com/api/classes/${classId}`, {
  ...
  const fetchExistingGrades = async () => {
    ...
    try {
      const response = await axios.get(`https://bethaniemiracle.com/api/debug/grades`, {
        params: {
          class_id: classId,
  ...
  const fetchAuthorizedSubjects = async () => {
    ...
    try {
      ...
      const response = await axios.get(`https://bethaniemiracle.com/api/bulletin-subjects/class/${classId}` , {
        headers: { Authorization: `Bearer ${token}` }
      });
  ...
  const fetchClassStudents = async () => {
    ...
    try {
      setLoading(true);
      const token = localStorage.getItem('token');
      const response = await axios.get(`https://bethaniemiracle.com/api/classes/${classId}/students`, {
        headers: { Authorization: `Bearer ${token}` }
      });
  ...
  const fetchCompositions = async () => {
    ...
    try {
      const token = localStorage.getItem('token');
      const response = await axios.get(`https://bethaniemiracle.com/api/compositions/class/${classId}`, {
        headers: { Authorization: `Bearer ${token}` }
      });
  ...
  const fetchExistingGradesForStudent = async () => {
    ...
    try {
      const requests = subjects.map((subj) =>
        axios.get(`https://bethaniemiracle.com/api/debug/grades`, {
          params: {
            class_id: classId,
            subject_id: subj.id,
            composition_id: selectedComposition
  ...
  const handleGradeChange = (studentId: number, value: string) => {
    ...
    if (isNaN(numericValue)) {
      ...
      // Marquer cet étudiant comme ayant une modification en cours
  ...
  const handleSaveGrades = async () => {
    ...
    try {
      ...
      const updatedStudentIds: number[] = [];
        ...
        for (const [studentId, grade] of Object.entries(grades)) {
          ...
          try {
            ...
            await axios.post('https://bethaniemiracle.com/api/teachers/grades', {
            ...
            // Marquer cet étudiant comme récemment mis à jour
      ...
      if (errorCount === 0) {
        ...
        // Déclencher le rafraîchissement des bulletins pour les étudiants mis à jour
    ...
    try {
      ...
      for (const subj of subjects) {
        if (subjectGrades[subj.id] !== undefined || existingSubjectGrades[subj.id]) {
          try {
            await axios.post('https://bethaniemiracle.com/api/teachers/publish-grades', {
              class_id: parseInt(classId),
              subject_id: subj.id,
              composition_id: selectedComposition
            }, { headers: { Authorization: `Bearer ${token}` } });
                        ...
                        <TableRow key="no-student">
                          <TableCell colSpan={5} align="center">
                            <Typography variant="body2" color="text.secondary" sx={{ py: 4 }}>
                              📚 Étudiant "{preSelectedStudentName}" non trouvé dans cette classe.
                              <br />
                              Veuillez vérifier que l'étudiant est bien inscrit pour cette année scolaire.
                            </Typography>
                          </TableCell>
                        </TableRow>
                      ];
