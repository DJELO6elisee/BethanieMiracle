...
const menuItems = [
  ...
  { 
    ...
    roles: ['admin', 'informaticien'] // Seul l'admin peut inscrire des enseignants
  },
  { 
    text: 'Classes', 
    icon: <ClassIcon />, 
  ...
  { 
    ...
    roles: ['admin'] // Seul l'admin peut gérer les médias
  },
  { 
...
const SecretarySidebar = () => {
  ...
  const fetchUserRole = async () => {
    try {
      ...
      const response = await axios.get('https://bethaniemiracle.com/api/auth/profile', {
        headers: {
  ...
  return (
    ...
    >
      <Box sx={{ p: 3, display: 'flex', alignItems: 'center', gap: 2, background: 'rgba(255,255,255,0.06)' }}>
        <img src="/img/pages/vrailogo.jpg" alt="Logo" style={{ width: 100, height: 48, borderRadius: 12, boxShadow: '0 2px 8px rgba(0,0,0,0.08)' }} />
